...
-- NUEVO (Sprint 3): columna "activo" para poder desactivar una especialidad
-- desde el panel de mantenimiento sin borrarla físicamente (así no se
-- rompen los médicos/citas que ya la referencian). DEFAULT 1 hace que
-- todas las especialidades existentes queden activas automáticamente.
...
-- =========================================================
-- Antes no existía ningún usuario con el rol "Administrador" (id_rol = 3),
-- por lo que el panel /admin no se podía probar. Se usa la MISMA
-- contraseña hasheada que ya usa todo el equipo para las demás cuentas de
-- prueba (mismo @pass de Sprint 2), así que el login funciona con la
-- misma clave que ya conocen.
INSERT INTO Usuarios (correo, password_hash, id_rol, activo)
...
-- =========================================================
-- Se agrega una cita ya "Atendida" con su historia clínica asociada, para
-- que el profesor pueda ver el módulo de Historia Clínica y los Reportes
-- del panel admin funcionando con datos reales desde el primer momento,
